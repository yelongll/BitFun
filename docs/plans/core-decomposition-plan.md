...
- 为高风险迁移建立迁移前快照测试：tool registry 清单与顺序、dynamic provider metadata、snapshot wrapper、MCP wire shape、remote-connect 消息字段、miniapp permission policy、function-agent 输入输出。
...
- MCP dynamic tools、MCP adapter、server process / transport runtime 外移。
...
- 当前分支保持小粒度：只包含 boundary check、facade/product runtime 注释、阶段文档，以及 Git / remote-ssh / MCP 的纯 owner crate helper 归属迁移；不继续混入 remote-connect、MCP dynamic tools、tool registry、miniapp 或 function-agent runtime 迁移。
...
- 质量边界：本阶段只证明已拆 owner crate 不依赖回 `bitfun-core`，并证明已迁移的 Git / remote-ssh / MCP 纯 helper 旧路径或调用点仍通过当前 Rust workspace 检查；不声明 remote connect、MCP dynamic tools、snapshot wrapping、miniapp/function-agent runtime 的外移完成。
