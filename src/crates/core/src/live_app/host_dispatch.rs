//! Host-side dispatch for LiveApp framework primitives (`shell.exec`, `fs.*`, `os.info`,
//! `net.fetch`).
//!
//! Why this exists
//! ---------------
//! The original LiveApp design routed every `app.*` call through a Bun/Node Worker
...
//! so LiveApps that only use `app.shell.exec` / `app.fs.*` / `app.net.fetch` can run
//! with `permissions.node.enabled = false` and no JS Worker at all.
//!
//! Routing rules (must match `useLiveAppBridge.ts`):
...
use crate::live_app::types::LiveAppPermissions;
...
///
/// `storage.*` is intentionally excluded: it is routed through `LiveAppManager` from the
