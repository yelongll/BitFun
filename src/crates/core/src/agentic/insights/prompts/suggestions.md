Analyze this 空灵语言 usage data and suggest improvements.

## 空灵语言 FEATURES REFERENCE (pick from these for features_to_try):
...
3. **MCP Servers**: Connect 空灵语言 to external tools, databases, and APIs via Model Context Protocol.
...
5. **CLI Exec (Headless)**: Run 空灵语言 non-interactively from scripts and CI/CD pipelines.
   - How to use: `空灵语言 exec "fix lint errors" --tools "Edit,Read,Bash"`
   - Good for: CI/CD integration, batch code fixes, automated reviews
   - CI/CD examples:
     ```bash
     # Pre-commit hook: auto-fix lint errors
     空灵语言 exec "fix all lint errors in staged files" --tools "Edit,Read,Bash"

     # PR review bot
     空灵语言 exec "review changes in this PR for security issues" --tools "Read,Grep,Glob"

     # Automated documentation
     空灵语言 exec "update API docs for all changed endpoints" --tools "Read,Edit,Glob"
     ```
...
RESPOND WITH ONLY A VALID JSON OBJECT:
{
  ...
  "features_to_try": [
    {"feature": "Feature name from 空灵语言 FEATURES REFERENCE above", "description": "What it does", "example_usage": "Actual command or config to copy", "benefit": "Why this would help YOU based on your sessions"}
  ],
  "usage_patterns": [
...
IMPORTANT for features_to_try: Pick 2-3 from the 空灵语言 FEATURES REFERENCE above. Include concrete, copy-pasteable example_usage for each. Tailor the benefit to the user's actual workflow patterns.
