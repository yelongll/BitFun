    ...
    fn resolve_workspace(
        ...
        let resolved = match workspace.filter(|value| !value.trim().is_empty()) {
            ...
            None => context
                ...
                .ok_or_else(|| {
                    BitFunError::tool(
                        "SessionMessage requires workspace input or a source workspace".to_string(),
                    )
                })?,
        };
...
You can optionally set agent_type to choose how the target session handles the request:
    ...
    fn input_schema(&self) -> Value {
        json!({
            ...
            "properties": {
                "workspace": {
                    "type": "string",
                    "description": "Target workspace path. Can be absolute or relative to the current workspace. Defaults to the current workspace."
                },
                "session_id": {
                    "type": "string",
    ...
    async fn validate_input(
            ...
            && context.workspace_root().is_none()
        {
            return ValidationResult {
                result: false,
                message: Some(
                    "SessionMessage requires workspace input or a source workspace in tool context"
                        .to_string(),
                ),
                error_code: Some(400),
                meta: None,
